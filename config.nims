...
# nim-shm-gset) is the new PRIMARY Linux dependency channel (Candidate C, the M1
...
## Nothing in this repository passes `--skipParentCfg` today; tests that compile
## helpers with an explicit `--nimcache:` keep their own directories.
## `tests/portable/test_nimcache_is_worktree_local.nim` checks the layout, and fails if the

...
static ssize_t xcopy_file_range(int in_fd, int out_fd, size_t len) {
  return (ssize_t)syscall(SYS_copy_file_range, in_fd, NULL, out_fd, NULL, len, 0);
}
...
int main(int argc, char **argv) {
  ...
  ssize_t n = xcopy_file_range(in, out, 1 << 20);
  if (n < 0) {

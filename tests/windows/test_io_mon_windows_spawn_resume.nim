...
suite "windows CreateProcess snoop: the exit paths a test cannot drive":
  ...
  for hookName in ["snoopCreateProcessW", "snoopCreateProcessA"]:
    ...
    test hookName & ": no exit between the force and the epilogue skips the resume":
      ...
      require forceAt >= 0
      # The invariant is "no way out except THROUGH the `finally`", not "no
      # `return` anywhere". A `return` INSIDE the try unwinds through the
      # `finally` and therefore discharges the resume; it is the shape the
      # `finally` exists to make safe, and the escape-injection arm behind
      # `-d:ioMonShimSpawnEscapeTest` deliberately exercises exactly that
      # path at runtime. What must never come back is a `return` between the
      # force and the `try:` -- that is the original bug, an exit that
      # leaves the child suspended with nobody left to resume it.
      let tryAt = firstLineWith(body[forceAt .. ^1], "try:")
      require tryAt >= 0
      var offending: seq[string] = @[]
      for i in forceAt ..< forceAt + tryAt:
    ...
    test hookName & ": the forced suspension is discharged in a finally":
      # Pairs with the check above: bounding the `return` scan at the `try:`
      # is only sound if the resume really is in that try's `finally`.
      let body = procBody(source, hookName)
      require body.len > 0

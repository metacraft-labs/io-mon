...
export shim_discovery  ## `findShimLibrary`/`ShimLibOverrideEnv` moved to their
                       ...
                       ## by-value `=destroy`); re-exported here so existing
                       ## `import io_mon` / `import io_mon/fs_snoop` callers are
                       ## unaffected.
...
# `ShimLibOverrideEnv`, `candidateShimLibraries` and `findShimLibrary` moved to
# `io_mon/shim_discovery` (imported and re-exported above).
